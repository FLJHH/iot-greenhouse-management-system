...
void hongwaiwendu_parse(unsigned char * input_buff ,unsigned int input_len ,void (*callback)(int wendu))
{
	...
	if (len_to_Read > 0)
	{
		...
		while ((len_to_Read--) && (index < sizeof(hognwai_show_buff)))
		{
			...
			else if(temp_value == 0x45 && flag_state == 1)
			{
				data_state =3;
			}

			else if(temp_value == 0x04 && data_state == 3 )
			{
				hognwai_show_buff[0] = 0x5A;
				hognwai_show_buff[1] = 0x5A;
				hognwai_show_buff[2] = 0x45;
				hognwai_show_buff[3] = 0x04;
				index=4;
				data_state = 4;

			}
			
			else if(flag_state==1 && data_state==4 )
			{	
