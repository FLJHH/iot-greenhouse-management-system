...
// 功能：获取GPIO初始化模式AO_PA4的值
// 参数：无
// 返回值：unsigned int类型的值
unsigned int Get_GPIO_Init_Mode_AO_PA4(void)
{
	unsigned int value;
	static unsigned char flag_is_init = 0; // 初始化标志位
	if (flag_is_init == 0) // 如果未初始化
	{
		flag_is_init = 1; // 初始化标志位设为1
		Adc_Init_channel(4); // 初始化ADC通道4
	}

	value = Get_ADC_channel(4); // 获取ADC通道4的值
