...
void Adjust_Sensor(void)
{
	...
	{
		GetSensor_IdValue_int("bp1",1);
	}
	else
	{
		GetSensor_IdValue_int("bp1",0);
	}
	...
	// 步进电机
	GetSensor_IdValue_int("d1", MOTOR_Direction_Control);
	// 灯
	GetSensor_IdValue_int("d2", GlobalVar_Module_status_updata_value1);
	// 继电器
	GetSensor_IdValue_int("d3", GlobalVar_Module_status_updata_value2);
	// 风扇
	GetSensor_IdValue_int("d4", wen_value);
}
...
void detect_sensor(void)
{
	...
	GetSensor_Dht11("wendu1", "shidu1"); // dht11温湿度

	GetSensor_Guangqiang("guangqiang1"); // 光强

	GetSensor_Turang("turang1"); //土壤

	Adjust_Sensor(); // 传感器调节判断

	// 发送完成，调用protocol_send_complete函数，传入参数为send_protocol_uart和user_send_data
...
void uart_recv_IdValue(const char *id, const char *value)
{
	...
	if (protocol_compare(id, "deng1")) // 判断控制指令
	{
		// 判断是否需要控制
		if (GlobalVar_flag_auto_adjust_wendu)
	...
	else if (protocol_compare(id, "duoji1")) // 判断控制指令
	{
		// 判断是否需要控制
		if (GlobalVar_flag_auto_adjust_wendu)
			return; // 自动调节模式下直接返回，防止对自动调节造成干
	...
	// 如果协议为"jdq1"
else if (protocol_compare(id, "jdq1"))
	{
		// 如果全局变量flag_auto_adjust_wendu为1
		if (GlobalVar_flag_auto_adjust_wendu)
			return;
		...
		if (protocol_compare(value, "on"))
		{
			...
			// 更新全局变量Module_status_updata_value2的值为1
		...
		else
		{
			...
			// 更新全局变量Module_status_updata_value2的值为0
